...
**_Important:_** There will not be a MacOS build for now. There might be one maybe in the far future.
...
1. Lost 5 days of brain cells
2. "COM3" is not same as "\\\\.\\COM3"
...
4. Size changing of table and graph moved to view.
5. Added a SeparatorMenuItem
...
When hovering over points on the scatterplot, the points will show their time and unit2.
...
4. Basepaths:
    1. MacOS: /Users/{user}/Library/Application Support/miramarwaterjets/FloatDataVisualizer/{release_version}
    2. Win11: C:\Users\{user}\AppData\Local\miramarwaterjets\FloatDataVisualizer\{release_version}
    3. Linux: {filepath}

### Virtual Threads

1. The app originally had about 4-5 Platform/OS Threads. This was then changed to have ~3 Platform Threads. 2 of the Platform Threads were changed to Virtual Threads.
...
2. [Bug] App when it didn't find the serial port was stuck and showed no ui or in the dock. This was only found in
...
3. [Fix] Add a check to see if the port exists.
    ...
    4. Else if Serial Port found, break from loop
7. [Bug] The app when sends the end-flag, throws "Unable to readLine()" exception.
8. [Fix] break from loop and interrupt Data Receiver Thread.
